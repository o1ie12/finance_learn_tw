...
/**
 * The one screen behind both 學習 and 模擬.
 *
 * These are not two pages. They are the same route map at two depths, which
 * is why this is one component with a variant rather than two layouts that
 * happen to look alike: the hero, the line chips, the strip map, the side
 * rail and the passport row are literally the same elements in both, so
 * switching reads as the content changing under you rather than a
 * navigation. Building 模擬 as its own flat list was the mistake — it broke
 * that continuity completely.
 *
 * The variant changes what a "stop" is. In learn, a line's stops are its
 * stations and then its simulation. In simulate, the only stop is the
 * simulation, so the same map renders one terminal per line and the same
 * hero points at the next unfinished simulation.
 */

export type NetworkVariant = "learn" | "simulate";

export default function RouteNetworkView({
  variant,
  student,
  progress,
  runsByLine,
  ...
  // started so "continue" still means continue.
  const simNext = (() => {
    const open = statuses.filter((s) => !s.simDone && s.line.sim.ready);
    if (open.length === 0) return null;
    const pick = open.find((s) => s.started) ?? open[0];
    return {
      line: pick.line,
      step: {
        kind: "sim" as const,
        href: `/line/${pick.line.slug}/simulation`,
        label: pick.line.sim.title,
      },
    };
  })();
  const next = simulateOnly ? simNext : nextActionAcrossLines(statuses);
  ...
  let bannerSubtitle = "";
  if (next) {
    if (!simulateOnly && next.step.kind === "station" && next.step.moduleNumber) {
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1.5 text-3xl font-black tracking-tight sm:text-4xl">
            {simulateOnly ? `${student.name} 的模擬` : `${student.name} 的路網`}
          </h1>
          ...
          <div className="mt-6">
            <LineNetworkPanel
              ...
              lines={statuses.map((s) => {
                const allStops = buildLineStations(
                  s.line,
                  progress,
                ...
                // one stop per line.
                const stations = simulateOnly
                  ? allStops.filter((st) => st.terminal)
                  : allStops;
